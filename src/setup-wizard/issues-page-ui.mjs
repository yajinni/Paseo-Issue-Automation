...
(function issuesSetupPage() {
  ...
  function render() {
    ...
    content().innerHTML = '<div class="paseo-grid">'
      + '<section class="setup-card"><h3>Issue selection</h3><p>Choose which open issues Paseo may consider. Invalid issues are still rejected by the installed issue contract.</p>'
      ...
      + '<section class="setup-card"><h3>Advanced options</h3>'
      ...
      + '<div class="inline-actions"><button class="action primary" id="issues-save" type="button">Save issue settings</button></div></section>'
      + '<section class="setup-card"><h3>Managed lifecycle labels</h3><p>Missing labels are created directly through GitHub only after final confirmation. Existing matching labels are reused without silently overwriting custom metadata.</p>' + labelRows() + '</section>'
      + '<section class="setup-card"><h3>Automation issue template</h3><p>' + escape(state.explanations?.template || '') + '</p><div class="notice">' + escape(template.message || 'Template preview is not available yet.') + '</div>'
    ...
    document.getElementById('issues-save')?.addEventListener('click', save);
  ...
  const observer = new MutationObserver(() => { if (onPage() && content() && !content().querySelector('#issues-save')) refresh(false); });
