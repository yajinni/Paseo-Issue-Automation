...
(function workspaceSetupPage() {
  ...
  function render() {
    ...
    content().innerHTML = '<div class="paseo-grid">'
      ...
      + '</section>'
      + (checkoutPage ? '<section data-ignore-required-checks class="' + cardClass(checkoutMissing) + '"><h3>Safe checkout discovery</h3><p>Setup checks only manager-known repositories, Paseo workspaces, and the manager-owned clone directory. Dirty user clones are shown but never altered.</p><div class="checklist">' + (candidateRows || '<div class="notice">No known checkout candidates were found.</div>') + '</div>'
