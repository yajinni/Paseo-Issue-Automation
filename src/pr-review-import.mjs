...
import { REVIEW_STAGES, REVIEW_WORKFLOW_OUTPUT_SCHEMA, REVIEW_WORKFLOW_PROMPT_VERSION, renderReviewWorkflowPrompt } from './review-workflow-prompts.mjs';
...
function validateAssociatedIssue(issue, issueNumber) {
...
export function importManagedPullRequest(root, input = {}, options = {}) {
  ...
  const prReader = options.prReader || managedPrSnapshot;
  const pr = prReader(root, parsed.pullRequestNumber);
  ...
  const issueReader = options.issueReader || defaultIssueReader;
  const issue = issueReader(root, configuredRepository, selectedIssue.issueNumber);
  validateAssociatedIssue(issue, selectedIssue.issueNumber);
