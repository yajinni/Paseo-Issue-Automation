---
name: Automated coding task
about: Define an issue clearly enough for autonomous implementation and independent review
title: ""
labels: "agent-ready"
assignees: ""
---

## Objective

<!-- State the concrete outcome this issue must achieve. -->

## Current behavior

<!-- Describe the relevant behavior today. -->

## Required behavior

<!-- Describe exactly what must change. -->

## Scope

<!-- List what is included. -->

## Out of scope

<!-- List related work that must not be included. -->

## Implementation direction

<!-- Add required constraints, boundaries, or preferred approach. Do not invent a second planning phase. -->

## Expected affected areas

<!-- List likely files, components, routes, services, schemas, or workflows when known. -->

## Dependencies

<!-- Use one dependency per line in either form: Blocked by #123 or Depends on #123. Write None when there are no dependencies. -->

## Privacy and security

<!-- State relevant privacy, authorization, secret-handling, and data-safety requirements. Use synthetic data only. -->

## Acceptance criteria

- [ ] <!-- Add verifiable completion criteria. -->

## Validation and checks

<!-- The issue author owns defining the checks appropriate for this task. Include focused tests, required repository commands, manual verification, or other evidence. -->

- [ ] <!-- Add an explicit validation or check. -->

## Documentation

<!-- State required documentation changes, or write None. -->

## Stop conditions

<!-- State conditions that require the automation to block instead of guessing, broadening scope, or taking unsafe action. -->
