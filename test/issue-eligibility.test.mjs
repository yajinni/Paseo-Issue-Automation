...
function config(mode = 'recommended-labels', excludedLabels = []) {
  return { baseBranch: 'main', issueSelection: { mode, excludedLabels } };
}

function deps(ok = true, numbers = []) {
...
test('blocked lowest-number candidate does not prevent the next eligible issue', () => {
  ...
  const result = evaluateIssueQueue('/repo', config(), {
    ...
    claimed: () => false,
    evaluateDependencies(_root, candidate) {
      return candidate.number === 101 ? deps(false, [88]) : deps(true);
    },
...
test('when the lower issue becomes unblocked it returns to the front of the queue', () => {
  const issues = [issue(102), issue(101)];
  const result = evaluateIssueQueue('/repo', config(), {
    issues,
    claimed: () => false,
  ...
  assert.equal(result.next.issue.number, 101);
});
...
test('all-open mode excludes configured labels, closed issues, pull requests, invalid contracts, and duplicate claims', () => {
  ...
  const result = evaluateIssueQueue('/repo', config('all-open', ['do-not-run']), {
    issues: [
      issue(100, { labels: [] }),
      issue(101, { labels: ['do-not-run'] }),
      issue(102, { labels: [], state: 'CLOSED' }),
      issue(103, { labels: [], extra: { isPullRequest: true } }),
      invalid,
      issue(105, { labels: [] }),
    ],
  ...
  assert.deepEqual(result.rejected.map((entry) => [entry.issueNumber, entry.kind]), [
    [101, 'excluded-label'],
    [102, 'closed'],
    [103, 'pull-request'],
    [104, 'invalid-contract'],
    [105, 'duplicate-claim'],
  ]);
});
...
test('dependency waiting is local state only and requires no blocked lifecycle label', () => {
  const saved = [];
  evaluateIssueQueue('/repo', config(), {
    issues: [issue(7)],
    claimed: () => false,
    evaluateDependencies: () => deps(false, [3]),
    runLoader: () => null,
    runSaver(_root, number, state) { saved.push([number, state]); return state; },
  });
  assert.equal(saved.length, 1);
  assert.equal(saved[0][1].phase, 'waiting-for-dependencies');
  assert.equal(saved[0][1].blockType, 'dependency');
  assert.equal(saved[0][1].status, 'waiting');
