...
test('controller fails closed when the independent reviewer subprocess fails', (t) => {
  if (process.platform === 'win32') return t.skip('acceptance fixture uses Unix executable shims');

  const fixture = mkdtempSync(path.join(os.tmpdir(), 'paseo-reviewer-failure-'));
  ...
  writeFileSync(path.join(root, 'README.md'), '# reviewer failure acceptance\n');
  git(root, 'add', 'README.md');
  git(root, 'commit', '--quiet', '-m', 'Initial fixture');
  git(root, 'remote', 'add', 'origin', remote);
  git(root, 'push', '--quiet', '-u', 'origin', 'main');

  git(root, 'checkout', '--quiet', '-b', 'issue-201-reviewer-failure');
  writeFileSync(path.join(root, 'accepted-change.txt'), 'candidate exact head\n');
  git(root, 'add', 'accepted-change.txt');
  git(root, 'commit', '--quiet', '-m', 'Add candidate change');
  git(root, 'push', '--quiet', '-u', 'origin', 'issue-201-reviewer-failure');
...
if (args[0] === 'issue' && args[1] === 'view') {
  out({ number: 201, title: 'Reviewer failure acceptance', body: 'Acceptance fixture', url: 'https://example.invalid/issues/201', comments: [], blockedBy: { nodes: [] }, blocking: { nodes: [] } });
  process.exit(0);
}
if (args[0] === 'issue' && (args[1] === 'edit' || args[1] === 'comment')) process.exit(0);
if (args[0] === 'pr' && args[1] === 'list') {
  out([{ number: 81, url: 'https://example.invalid/pull/81', isDraft: true, headRefOid: process.env.PASEO_ACCEPTANCE_HEAD, baseRefName: 'main', baseRefOid: process.env.PASEO_ACCEPTANCE_BASE, mergeable: 'MERGEABLE', mergeStateStatus: 'CLEAN', statusCheckRollup: [] }]);
  process.exit(0);
}
...
`;
  ...
  saveRun(root, 201, {
    issueNumber: 201,
    issueTitle: 'Reviewer failure acceptance',
    attempt: 1,
    status: LEGACY_LABELS.running,
    phase: 'coding',
    branch: 'issue-201-reviewer-failure',
    worktreePath: root,
    workspaceId: 'workspace-201',
    coderAgentId: 'coder-201',
    agentId: 'coder-201',
    events: [],
    activity: [],
  ...
  const result = spawnSync(process.execPath, [controller, root, '201'], {
    cwd: root,
    encoding: 'utf8',
    env: {
      ...process.env,
      ...
      PASEO_ACCEPTANCE_HEAD: head,
      PASEO_ACCEPTANCE_BASE: base,
  ...
  const failed = loadRun(root, 201);
  assert.equal(failed.status, LEGACY_LABELS.failed);
  assert.equal(failed.phase, 'failed');
  assert.match(failed.reason || '', /simulated reviewer subprocess failure/i);
  assert.equal(failed.prNumber, 81);
  assert.equal(failed.approvedCommit ?? null, null);

  const validation = (failed.events || []).filter((event) => event.event === 'validation-summary');
  const reviews = (failed.events || []).filter((event) => event.event === 'review');
  assert.equal(validation.length, 1);
  assert.equal(validation[0].result, 'PASS');
  assert.equal(validation[0].commit, head);
